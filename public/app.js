...
function formatPrice(price) {
  ...
  if (typeof price === "string") {
    const parts = price.split("-").map((s) => s.trim());
...
function renderServices() {
  ...
  Object.keys(salonData.services || {}).forEach((key) => {
    const el = renderGroup(key);
    if (el) groupsRoot.appendChild(el);
  });
}
...
// Mobile 100vh fix: set --vh to viewport height in px units
...
function heroCanvas() {
  ...
  function step() {
    ...
    particles.forEach((p) => {
      ...
      ctx.beginPath();
      ctx.arc(x, y, p.r + (Math.hypot(dx,dy) < 0.12 ? 0.6 : 0), 0, Math.PI * 2);
      ctx.fill();
    });
...
// Parallax on hero content and spline using mouse position
function initHeroParallax(){
  ...
  window.addEventListener('mousemove', (e)=>{
    ...
    parallaxEls.forEach((el, idx)=>{
      const depth = idx === 0 ? 6 : 10; // hero-inner shallower, spline deeper
      el.style.transform = `translate3d(${dx * depth}px, ${dy * depth}px, 0)`;
    });
  });
}
window.addEventListener('DOMContentLoaded', initHeroParallax);
...
function initAccordion(){
  document.querySelectorAll('.acc-toggle').forEach((btn)=>{
    btn.addEventListener('click', ()=>{
      ...
      // close all others to behave like single-open accordion
      document.querySelectorAll('.acc-toggle').forEach((b)=>{
        ...
        if (b !== btn) {
          ...
          if (p) p.hidden = true;
          const ico = b.querySelector('.acc-icon');
